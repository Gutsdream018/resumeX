...
import {
  ...
  detectSections,
  ExtractionError,
} from '../src/services/textExtractor.js';
...
describe('Text Extraction and Sanitization', () => {
  ...
  it('should throw an ExtractionError for empty buffer', async () => {
    ...
    await expect(
      extractTextFromBuffer(emptyBuffer, 'text/plain', 'empty.txt')
    ).rejects.toThrow(ExtractionError);
  });

  it('should accept and extract short resume text without arbitrary length limits', async () => {
    const shortBuffer = Buffer.from('John Doe, programmer.', 'utf-8');
    const result = await extractTextFromBuffer(shortBuffer, 'text/plain', 'short.txt');
    expect(result.text).toBe('John Doe, programmer.');
  ...
  it('should throw an ExtractionError for unsupported file extension', async () => {
    ...
    await expect(
      extractTextFromBuffer(buffer, 'application/zip', 'archive.zip')
    ).rejects.toThrow('Unsupported file format (.zip)');
  });
