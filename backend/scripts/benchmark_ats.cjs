...
async function runBenchmark() {
  ...
  const { runComprehensiveAtsPipeline } = await import('../dist/engine/pipeline.js').catch(async () => {
    // If running in development with tsx or direct esm
    return await import('../src/engine/pipeline.js');
  });
